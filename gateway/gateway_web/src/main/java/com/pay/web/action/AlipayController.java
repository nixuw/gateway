...
public class AlipayController {
	...
	@Autowired
	@Qualifier("alipaySyncCallbackService")
	private ThirdCallBackService<GatewayAlipayResponse> callback;
	
	@Autowired
	@Qualifier("alipayASyncCallbackService")
	private ThirdCallBackService<GatewayAlipayResponse> notify;
	...
	public String callback(HttpServletRequest request, ModelMap model) {
		...
		try {
			...
			logger.error("处理支付宝同步回调失败",e);
			return "error";
		}
		
		model.put("orderNo", resp.getOutTradeNo());
